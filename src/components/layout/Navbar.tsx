...
export default function Navbar({ user: propUser }: { user?: any }) {
  ...
  const navLinks = [
    ...
    { name: 'Mind Maps', hasDropdown: false, path: '/ai-hub' },
    ...
    <nav className="w-full flex items-center justify-between px-8 py-6 bg-transparent font-sans relative z-50">
      ...
      <Link href="/" className="flex items-center gap-3 cursor-pointer group">
        <div className="p-1 rounded-xl transition-all duration-300 shadow-sm overflow-hidden bg-white/10">
          <Image 
            src="/moana-ai-logo.png"  // FIXED PATH: Direct access to public root
            alt="Moana AI Logo" 
            width={40}                 // Required for Next.js Image
            height={40}                // Required for Next.js Image
            className="rounded object-contain" // Removed mix-blend-multiply to ensure visibility
          />
        </div>
      ...
      <div className="hidden md:flex items-center bg-white/60 backdrop-blur-xl px-1.5 py-1.5 rounded-full shadow-lg shadow-emerald-100/50 border border-emerald-100/50">
        ...
        <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-emerald-800 hover:bg-emerald-50 rounded-full transition-colors cursor-pointer">
