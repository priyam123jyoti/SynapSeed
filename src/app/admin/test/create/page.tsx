      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit space-y-4">
            ...
            <button 
              onClick={handleAiGeneration} disabled={isGenerating || isParsingPdf || !aiContext.trim()}
                ...
                {stagedQuestions.map((q, idx) => (
                  <div key={q.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <button onClick={() => setStagedQuestions(stagedQuestions.filter(item => item.id !== q.id))} className="text-rose-500 hover:text-rose-700">
                        <Trash2 size={16} />
                      </button>
                    </div>
